...
if chk_list "myShell" "${shlList[@]}"; then
    ...
    print_log -sec "SHELL" -stat "skipped" "keeping existing shell (bash/zsh/fish not installed)..."
    exit 0
fi

# Setup prompt for bash users using improved manager
...
if pkg_installed zsh; then
    prompt_timer 120 "Pre install zsh plugins using oh-my-zsh? [y/n/s to skip] "
    PROMPT_INPUT="${PROMPT_INPUT:-y}"
    if [[ "${PROMPT_INPUT}" == "y" || "${PROMPT_INPUT}" == "yes" || "${PROMPT_INPUT}" == "" ]]; then
